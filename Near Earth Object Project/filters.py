...
class DateFilter(AttributeFilter):
    """Compare approach date to the value supplied."""

    @classmethod
    def get(cls, approach):
        """Replace get method in `AttributeFilter`."""
        return approach.time.date()


class DistanceFilter(AttributeFilter):
    """Compare approach distance to the value supplied."""
...
class VelocityFilter(AttributeFilter):
    """Compare approach velocity to the value supplied."""
...
class DiameterFilter(AttributeFilter):
    """Compare approach diameter to the value supplied."""
...
class HazardousFilter(AttributeFilter):
    """Compare approach hazardous value to the value supplied."""
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    if date:
        date_eq = DateFilter(operator.eq, date)
        filters += (date_eq,)
    if start_date:
        date_ge = DateFilter(operator.ge, start_date)
        filters += (date_ge,)
    if end_date:
        date_le = DateFilter(operator.le, end_date)
        filters += (date_le,)
    if distance_min:
        distance_ge = DistanceFilter(operator.ge, distance_min)
        filters += (distance_ge,)
    if distance_max:
        distance_le = DistanceFilter(operator.le, distance_max)
        filters += (distance_le,)
    if velocity_min:
        velocity_ge = VelocityFilter(operator.ge, velocity_min)
        filters += (velocity_ge,)
    if velocity_max:
        velocity_le = VelocityFilter(operator.le, velocity_max)
        filters += (velocity_le,)
    if diameter_min:
        diameter_ge = DiameterFilter(operator.ge, diameter_min)
        filters += (diameter_ge,)
    if diameter_max:
        diameter_le = DiameterFilter(operator.le, diameter_max)
        filters += (diameter_le,)
    if hazardous or hazardous is False:
        hazardous_eq = HazardousFilter(operator.eq, hazardous)
        filters += (hazardous_eq,)
    return filters
